					...
					@if(isset($data))
						...
						<tr>
							...
							<td >
								...
								<a href="{{route('payroll.show',$value->id)}}"><button class="btn btn-success  pull-left" title="download detail "><i class="fas fa-file-download"> </i> </button></a>

                                <form action="{{route('payroll.destroy',$value->id)}}"  onclick="return confirm('Are you sure you want to delete ?')" method="POST" >

                                              @method('delete')
                                              @csrf
										<button type="submit" title="delete" class="btn btn-danger"><i class="fas fa-times"> </i></button>
								    </form>
							</td>
						</tr>
						@endforeach
